function [KG]=NonGeometryStiffnessPlateAVE(FEM,Laminate,Stru)
% average stress in each element at different layers
...
% Loop for each element, element stiffness,

for e=1:FEM.numberElements
    
    layer=size(Laminate.thickness,2);
    
    stress0=[sum(FEM.stress(e,1,1:layer))/layer,sum(FEM.stress(e,3,1:layer))/layer;
        sum(FEM.stress(e,3,1:layer))/layer,sum(FEM.stress(e,2,1:layer))/layer];
    ...
    for ee=1:size(GaussWeights,1)
        ...
        KG(NodeIndices+numberNodes,NodeIndices+numberNodes)=...
            KG(NodeIndices+numberNodes,NodeIndices+numberNodes)+...
        ...
        KG(NodeIndices+numberNodes*2,NodeIndices+numberNodes*2)=...
            KG(NodeIndices+numberNodes*2,NodeIndices+numberNodes*2)+...
