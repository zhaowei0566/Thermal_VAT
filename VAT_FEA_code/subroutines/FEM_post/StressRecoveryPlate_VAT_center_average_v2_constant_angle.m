function [stress_global,strain_global]=StressRecoveryPlate_VAT_center_average_v2_constant_angle(FEM,Laminate,Mat,Stru,T0T1,center,physical_length)
...
for layer=1:size(T0T1,1)
    ...
    for elem=1:FEM.numberElements
        ...
        for gauss_point_num=1:size(GaussWeights,1)
            ...
            Elemental_centerX = sum(FEM.nodeCoordinates_label( NodeIndices,2))/length( NodeIndices);
            %             Elemental_centerX  = shape'*FEM.nodeCoordinates( NodeIndices,1);
            %             Elemental_centerY  = shape'*FEM.nodeCoordinates( NodeIndices,2);
            ...
            theta = VAT_fiber_ply_angle_1D(T0,T1,Elemental_centerX,center,physical_length);
