...
%     deformUZ_2(ActiveBendDOF)=FEM.modeshape(1:bendModeNo,modeNo+1);
...
%     z3_2=griddata(Xcoord,Ycoord,deformUZ_2,x3,y3,'v4')*factor;
    
    % ZQ=griddata(Xcoord,Ycoord,Zcoord,deformUZ,x3,y3,z3,'linear');
    hf=figure;
    axes1 = axes('Parent',hf,'YTick',[0 Stru.width  ],'XTick',[0 Stru.length],...
...
%     
%      subplot(2,1,2);
%     surf(x3,y3,(z3_2)*scalefactor ,'FaceColor','interp',...
%         'LineStyle','none',...
%         'EdgeColor','none',...
%         'FaceLighting','phong');hold on;
%     colorbar('FontSize',16)
...
%     
    ...
    for kk=1:size(XXstiffener,1)
        ZZstiffener(kk,:)=ones(1,size(XXstiffener(1,:,kk),2))*Stiffener.height;
